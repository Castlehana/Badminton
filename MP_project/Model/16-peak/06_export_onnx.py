# scripts/06_export_onnx.py  (T=16 학습 ckpt 대응 버전)
...
# - 주의: target_T는 ckpt 메타를 그대로 사용합니다. (05에서 TARGET_T=16으로 저장되어 있어야 함)
...
TARGET_T_DEFAULT = 33     # ckpt에 없을 때만 사용. 정상 경로에선 ckpt의 target_T=16을 읽음.
...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # causal crop (padding으로 늘어난 오른쪽 잘라내기)
        T = x.shape[-1]
        out = out[..., -T:]
...
def extract_meta(ckpt: dict):
    ...
    # 추가 메타가 있을 경우(예: peak_pos/causal/include_peak)도 그대로 보존
    extra = {}
    for k in ['peak_pos','causal','include_peak']:
        if k in ckpt:
            extra[k] = ckpt[k]
...
def main():
    ...
    # (선택) 강제 오버라이드 옵션 — ckpt 메타가 이상할 때만 사용
    ap.add_argument('--force_target_T', type=int, default=None, help='메타의 target_T를 강제로 덮어씀')
    ap.add_argument('--force_feat_dim', type=int, default=None, help='메타의 feat_dim을 강제로 덮어씀')
    args = ap.parse_args()
    ...
    # 더미 입력 (B=1, T, D) — 고정 길이 추론(동적축 미사용)
    ...
    torch.onnx.export(
        ...
        dynamic_axes=None,  # 고정 길이(T=16 또는 메타의 target_T) 가정

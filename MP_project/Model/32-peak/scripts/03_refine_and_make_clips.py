# scripts/03_refine_and_make_clips.py  (T=32, causal right-aligned, peak EXCLUDED)
...
# 하이퍼파라미터 / 상수
# =========================
# (변경) T=32: 피크 '직전' 32프레임만 사용(피크 프레임 미포함)
T = 32
...
def process_pair(lmk_path, evt_csv, out_dir):
    ...
    for e in evs:
        ...
        # (변경) 피크 직전 T=32 슬라이스 (피크 미포함)
        idxs = _slice_T_ending_at_peak(k, len(t), T, include_peak=False)
        if idxs is None:
            continue

        seg_n = dfn.iloc[idxs].reset_index(drop=True)
        X = _make_features(seg_n)                 # (T=32, 16)

# scripts/05_train_tcn.py  (T=33, causal right-aligned, PEAK INCLUDED)
# - 입력: dataset/clips/raw/class=*/*.npz (X:(T,D), y(int), meta(json))
# - 윈도우: 피크 '이전 32프레임 + 피크 1프레임' = 총 33프레임 (우측정렬, causal)
# - 클래스: ['Clear','Drive','Drop','Under','Hairpin','Idle'] (6클래스)
# - 표준화: z-score (train 통계로만 적합)
# - 증강: 시간축 roll(±2) [train에서만]  ← T=33 문맥 안정성에 맞춤
...
# - 결과 저장: best.pt (메타: classes, zscore_mu/std, target_T=33, feat_dim, io tensor names)
...
def fit(args):
    ...
    # 데이터셋/로더
    ds_tr = ClipsDataset(train_files, z_mu=mu, z_std=std, train=True,  aug_roll=2)
    ...
    for ep in range(1, args.epochs+1):
        ...
        if f1 > best_f1:
            ...
            ckpt = {
                ...
                'feat_dim': FEAT_DIM,
                'target_T': TARGET_T,         # ← 33으로 저장
                'input_name': INPUT_NAME,
                'output_name': OUTPUT_NAME,
                'seed': SEED,
                # (선택) 정렬 메타를 추가로 박아두면 디버깅 유리

...
def _slice_T_centered_at_peak(peak_idx, total_len, BEFORE, AFTER):
    start = peak_idx - BEFORE
    end   = peak_idx + AFTER + 1   # +1 → Python 슬라이스 상 포함
...
def _gen_idle_clips(dfn, t, v_wrist, v_elbow, rec_intervals, n_target, out_dir, lmk_path, evt_csv, meta_base):
    ...
    for k in picked:
        idxs = _slice_T_centered_at_peak(k, len(t), BEFORE, AFTER)
        if idxs is None:
            continue
...
def process_pair(lmk_path, evt_csv, out_dir):
    ...
    for e in evs:
        ...
        meta = {
            ...
            'frames_before_peak': int(BEFORE),
            'frames_after_peak': int(AFTER),
            'peak_centered': True,
        }

...
# - meta에 subject(S###), video(베이스파일명), peak_pos="last" 포함
# - ★ 변경점: 피크 프레임을 "마지막 프레임"으로 포함하는 끝-정렬(end-aligned) 슬라이스
...
def _slice_T_end_at_peak(peak_idx, total_len, T):
    """피크 프레임을 마지막으로 포함하는 끝-정렬(end-aligned) 구간 [start, peak]"""
    start = peak_idx - (T - 1)
    end = peak_idx + 1     # 슬라이스 상의 포함 끝
...
def process_pair(lmk_path, evt_csv, out_dir):
    ...
    for _, row in ev.iterrows():
        ...
        # ★ 피크를 마지막 프레임으로 포함하는 끝-정렬 슬라이스
        idxs = _slice_T_end_at_peak(k, len(t), T)
        if idxs is None: 
            continue

        seg_n = dfn.iloc[idxs].reset_index(drop=True)
        X = _make_features(seg_n)                 # (T,16)

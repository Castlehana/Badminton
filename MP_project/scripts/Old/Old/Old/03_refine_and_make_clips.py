...
# - meta에 subject(S###), video(베이스파일명) 포함
# - 변경점: 02번 출력에 맞춰 관절 구성 수정(0,23 제거 / 13 추가), front 정의를 어깨 중점 기준으로 변경
...
def _torso_norm(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = {}
    # 필요 관절만 정규화: 11,12,13,14,16,24  (0,23 제거)
    need = [IDX_L_SHOULDER, IDX_R_SHOULDER, IDX_L_ELBOW, IDX_R_ELBOW, IDX_R_WRIST, IDX_R_HIP]
    for j in need:
...
def _compute_phi_shoulder(arr):
    # 어깨 각: 왼→오른 어깨 벡터가 있으면 사용, 없으면 토르소(12→24) 기반
...
def _make_features(seg_n: pd.DataFrame) -> np.ndarray:
    ...
    # 각도/거리들 (오른어깨 12, 오른팔꿈치 14, 오른엉덩이 24)
    dx = arr['x16n'] - arr['x12n']; dy = arr['y16n'] - arr['y12n']
    theta = np.arctan2(dy, dx)
    dtheta = np.gradient(theta, t, edge_order=1)

    # front: 코(0) 미사용 → 어깨 중점(mid-shoulders) 기준으로 오른손목의 좌/우
    x_mid_sh = (arr['x11n'] + arr['x12n']) / 2.0
    ...
    d_wh = np.hypot(arr['x16n']-arr['x24n'], arr['y16n']-arr['y24n'])  # wrist-hip (right)

    phi_shoulder = _compute_phi_shoulder(arr)
    theta_rel = _wrap_pi(theta - phi_shoulder)

    # 팔꿈치 각(오른쪽): shoulder(12)-elbow(14)-wrist(16)
...
def _slice_T(center_idx, total_len, T):
    half = T // 2
    start = center_idx - (half - 1)
    end = start + T
...
def process_pair(lmk_path, evt_csv, out_dir):
    ...
    # 필수 포인트 가용성 체크 (오른손목/오른팔꿈치 중 하나라도 충분히 있어야 함)

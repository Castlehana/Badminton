...
# - 증강: 시간축 roll(±2) 학습시에만
# - 스케줄러: ReduceLROnPlateau(mode='max')  # verbose 옵션 제거(경고 방지)
# - 결과 저장: best.pt (프로젝트 루트: {'model','mu','std','feat_dim','classes','target_T'})
...
# ★ 센터 정렬(피크 중앙) 학습을 위해 T=33 고정
TARGET_T = 33        # ← 32 → 33
TARGET_D = 16

# ---------------- 유틸 ----------------
def _resample_time(x, target_T=TARGET_T):
    """(T,D) → (target_T,D) 선형보간. 03에서 만들어진 33프레임이 아니어도 방어적으로 처리."""
    T, D = x.shape
    ...
    for ep in range(1, EPOCHS+1):
        ...
        if va_acc > best_acc:
            ...
            best_state = {
                ...
                'classes': CLASSES,
                'target_T': TARGET_T,   # ← 메타에 T 저장(센터 정렬 추론과 일치)
            }

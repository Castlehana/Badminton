...
# - z-score 통계(mu/std)와 클래스 목록, target_T(슬라이스 길이) 등을 meta JSON으로 함께 저장
#
# 예)
#   python scripts/06_export_onnx.py
#   python scripts/06_export_onnx.py --ckpt best.pt --out exports/tcn.onnx --meta exports/tcn_meta.json --opset 17
#
# I/O:
...
class TimeAvgPoolKeepD(nn.Module):
    """[B, C, D, T] -> 시간축 평균 -> [B, C, D, 1]
       학습 head의 AdaptiveAvgPool2d((feat_dim,1))에서 T만 1로 만드는 역할과 동치(D는 유지)."""
    def forward(self, x):
...
class TCN(nn.Module):
    def __init__(self, feat_dim, num_classes):
        ...
        self.head = nn.Sequential(
            TimeAvgPoolKeepD(),                 # head.0 (T 평균)
            nn.Flatten(1),                      # head.1
            nn.Linear(128 * feat_dim, 256),     # head.2
            nn.ReLU(),                          # head.3
            nn.Dropout(0.30),                   # head.4
...
def infer_target_T_from_stats(mu, std, fallback=32):
    """mu/std가 (1,T,D) 형태일 경우 T를 추정. 아니면 fallback."""
    try:
        arr = np.asarray(mu)
        if arr.ndim == 3:
...
def save_meta(meta_path, mu, std, classes, target_T, feat_dim):
    meta = {
        ...
        "input_name": "clips",
        "output_name": "logits",
        "note": "z-score는 (X - mu)/std 형태로 적용; target_T는 학습/추론 시퀀스 길이(예: 33=피크 중심 정렬)"
    }
...
def main():
    ...
    ap.add_argument("--override_target_T", type=int, default=None, help="체크포인트의 target_T 대신 강제 설정")
    ...
    # target_T 결정 우선순위: --override -> ckpt['target_T'] -> mu/std에서 추정 -> 32
    if args.override_target_T is not None:
        target_T = int(args.override_target_T)
    elif "target_T" in ckpt:
        target_T = int(ckpt["target_T"])
    else:
        target_T = infer_target_T_from_stats(ckpt["mu"], ckpt["std"], fallback=32)
    ...
    # 메타 저장
    save_meta(args.meta, ckpt["mu"], ckpt["std"], classes,
              target_T=target_T, feat_dim=feat_dim)

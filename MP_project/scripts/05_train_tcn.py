...
class ClipsDataset(Dataset):
    def __init__(self, files, z_mu=None, z_std=None, train=False, aug_roll=2):
        self.files = files
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        # 모양 강제 (자동 감지된 TARGET_T 기반)
        if X.shape[0] != TARGET_T or X.shape[1] != FEAT_DIM:
            T0, D0 = X.shape
            # 시간 길이 보정
            if T0 < TARGET_T:
                pad = np.zeros((TARGET_T - T0, D0), dtype=np.float32)
...
# ====================== 학습 루프 ======================
def fit(args):
    global TARGET_T  # 자동 감지를 위해 global 사용
    ...
    # =====================================================================
    if TARGET_T is None:
        sample_X, _, sample_meta = read_npz(train_files[0])
        TARGET_T = int(sample_meta.get("T", sample_X.shape[0]))
        print(f"[META] Auto TARGET_T = {TARGET_T}")

    # =====================================================================
    # ====== ② BEFORE / AFTER 자동 감지 ====================================
    # =====================================================================
    frames_before = int(sample_meta.get("frames_before_peak", (TARGET_T - 1)//2))
    frames_after  = int(sample_meta.get("frames_after_peak",  (TARGET_T - 1)//2))
    peak_centered = bool(sample_meta.get("peak_centered", True))

    print(f"[META] BEFORE={frames_before}, AFTER={frames_after}, peak_centered={peak_centered}")
    ...
    # ---------------- Dataset ----------------
    ds_tr = ClipsDataset(train_files, z_mu=mu, z_std=std, train=True)
    ds_va = ClipsDataset(val_files,   z_mu=mu, z_std=std, train=False)
    ds_te = ClipsDataset(test_files,  z_mu=mu, z_std=std, train=False)
    ...
    train_loader = DataLoader(ds_tr, batch_size=args.batch, sampler=sampler)
    val_loader   = DataLoader(ds_va, batch_size=args.batch, shuffle=False)
    test_loader  = DataLoader(ds_te, batch_size=args.batch, shuffle=False)

    # ---------------- 모델 ----------------
    model = TCN(in_feat=FEAT_DIM, num_classes=NUM_CLASSES).to(device)
    optim = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, mode='max', patience=5, factor=0.5)
    criterion = FocalLoss()
    ...
    for ep in range(1, args.epochs+1):
        ...
        if f1 > best_f1:
            ...
            ckpt = {
                ...
                # 성능 기록
                'best_epoch': ep,
                'best_val_macroF1': float(f1),
                'best_val_loss': float(va_loss),
                'train_loss_at_best': float(tr_loss),

                # 데이터 크기
                'num_train_samples': len(ds_tr),
                'num_val_samples': len(ds_va),
                'num_test_samples': len(ds_te),

                # ===== ③ 클립 BEFORE/AFTER 저장 (자동 반영) =====
